...
{
	p.elem = (Elemtype*)malloc(size*sizeof Elemtype);
	p.rear = 0;//指向队尾元素的下一个位置
	p.front = p.rear;//front指向队头元素
	p.queuesize = size;
}
...
void EnQueue(Queue& p,Elemtype& e)//队尾插入，队尾指针+1
{
	if( (p.rear+1)%p.queuesize != p.front)//队列判满
	{
		p.elem[p.rear] = e;
...
Elemtype& DeQueue(Queue& p,Elemtype& e)//队头删除，队头指针+1
{
	if(p.rear != p.front)//队列判空
	{
		e = p.elem[p.front];
